...
class Hvf_Test:
            ...
            for r in range(10):
                ...
                if not (val.is_equal(exp_val)):
                    ...
                    string = (
                        ...
                        + str(r)
                        + ") - expected "
                        + exp_val.get_display_string()
                        + ", actual "
                        + val.get_display_string()
                    )
    ...
    def compare_test_duration(ref_data, test_data):
        ...
        try:
            ...
            bool = (int(ref_data_list[0]) == int(test_data_list[0])) and (
    ...
    def test_hvf_obj(test_name, reference_hvf_obj, test_hvf_obj, time_elapsed):
        ...
        if reference_hvf_obj.equals(test_hvf_obj):
            ...
            testing_data_dict["value_plot_vals"] = Hvf_Test.count_val_nonempty_elements(
                reference_hvf_obj.abs_dev_value_array
            ) + Hvf_Test.count_val_nonempty_elements(reference_hvf_obj.pat_dev_value_array)
            testing_data_dict["perc_plot_vals"] = Hvf_Test.count_perc_nonempty_elements(
                reference_hvf_obj.abs_dev_percentile_array
            ) + Hvf_Test.count_perc_nonempty_elements(reference_hvf_obj.pat_dev_percentile_array)
    ...
    def test_single_image(hvf_image):
        ...
        if serialization == serialization2:
            ...
            Logger.get_logger().log_msg(
                Logger.DEBUG_FLAG_SYSTEM, "FAILED serialization/deserialization consistency =============="
            )
        ...
        Logger.get_logger().log_msg(
            ...
            "Global CIGTS TDP Score: " + str(Hvf_Metric_Calculator.get_global_cigts_tdp_score(hvf_obj)),
        )
    ...
    def add_unit_test(test_name, test_type, ref_data_path, test_data_path):
        ...
        if not os.path.isdir(create_path_if_not_present):
            Logger.get_logger().log_msg(
                Logger.DEBUG_FLAG_SYSTEM, "Making new unit test directory: " + create_path_if_not_present
            )
