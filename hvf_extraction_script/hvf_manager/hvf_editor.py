            ...
            for j in range(0, size):
                ...
                if not (Hvf_Plot_Array.BOOLEAN_MASK_24_2[j][i]):
                    if new_plot_obj.icon_type == Hvf_Plot_Array.PLOT_PERC:
                        new_plot_obj.plot_array[j, i] = Hvf_Perc_Icon.get_perc_icon_from_char(
                            Hvf_Perc_Icon.PERC_NO_VALUE_CHAR
                        )
                    elif new_plot_obj.icon_type == Hvf_Plot_Array.PLOT_VALUE:
                        new_plot_obj.plot_array[j, i] = Hvf_Value.get_value_from_display_string(
                            Hvf_Value.VALUE_NO_VALUE_CHAR
                        )
