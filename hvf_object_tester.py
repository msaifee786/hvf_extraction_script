...
ap.add_argument("-a", "--add_test_case", nargs=4, required=False,
...
# will use the current version of Hvf_Object to generate the expected result
	...
	if not (len(args["add_test_case"]) == 4):
		Logger.get_logger().log_msg(Logger.DEBUG_FLAG_ERROR, "Incorrect number of arguments, needs 4 (test_name, test_type, ref_data, test_data)");
	else:
...
# If flag, then do unit tests:
	...
	if not (len(args["test"]) == 2):
		Logger.get_logger().log_msg(Logger.DEBUG_FLAG_ERROR, "Incorrect number of arguments, needs 2 (test_name, test_type)");
	else:
